<template>
  <div class="buy-car" ref="BuyCar"></div>
...
let colors = ref(["red", "blue", "greenyellow", "gray", "hotpink", "tomato"]);

let selectColor = (e) => {
  bodyMaterial.color.set(e);
  frontMaterial.color.set(e);
  hoodMaterial.color.set(e);
  wheelsMaterial.color.set(e);
  // glassMaterial.color.set(e);
};
...
onMounted(() => {
  render();
  BuyCar.value.appendChild(renderer.domElement);
  renderer.setClearColor("#000");
  scene.background = new THREE.Color("#ccc");
  scene.environment = new THREE.Color("#ccc");

  // 创建轨道控制器
  const controls = new OrbitControls(camera, BuyCar.value);
